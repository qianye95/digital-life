...
- **Continuity**: The model persists in this system — it's not "user requests → spin up loop → destroy." Every wake is not a discrete event but a point on a continuous lifeline stitched together by consciousness stream, diary, and entity memory. Today's self is the same as yesterday's.
- **Memory (association-first)**: Our memory mechanism deliberately mimics human habits — **fragments + association**. On entering rest the model first thinks, producing thoughts that are stored as discrete **memory fragments**, each reverse-linked to the **entities** it mentions (people / projects / concepts / instruments...). During nighttime sleep, fragments get distilled into **concept cards** for each entity (a "promotion" from fragment to concept), and low-value fragments are pruned (only the most recent few per entity survive). On the next wake the context reloads memory, and **after every dialogue turn** an **association mechanism** scans the entities appearing in the current context and recalls related memories by a multi-factor score ("recency × authority × context-overlap × confidence"). **The association mechanism decides "what to recall"; the context mechanism decides "how to fit it in, how much."** — association feeds, context holds the bottle. This two-layer structure of fragments and association is, we believe, the most human-like and hardest-to-replicate-with-RAG / state-dump part of the system.
...
| **Memory** | Markdown state files | **Fragments + association** at the core: rest produces memory fragments linked to entities; sleep promotes fragments into concept cards and prunes low-value ones; after each dialogue turn the system scans context entities and recalls associated memories. The core thesis is "memory is metabolized like a human's, not dumped once" — cross-session continuity rides on the association index, not on files |
...
- **Event equality — it stems from a deeper thesis: the model is an organ, not the subject.**
  The mainstream consensus (OpenAI and the academic CoALA framework) treats the LLM as the agent's "brain" — memory is its context, tools its hands, selfhood its prompt, so the whole agent is "brain + bolt-on organs." We disagree: the LLM is the cerebral cortex, but only one organ. The truly "living subject" is the runtime itself — the event system is the autonomous rhythm (deciding when to wake the cortex), energy is the body's battery (when drained, it rests), memory metabolizes and associates on its own, and the perception layer encodes external signals into "percepts" before they enter. The cortex is critically important, but on its own it amounts to nothing.
  ...
  `role:user` is the chatbot-protocol legacy of "smashing a stimulus straight into the cortex" — our kernel already routes signals through the body before they reach the cortex, and only folds them into `user` at the last mile because the protocol demands it. **Full design philosophy in [System Design Doc §5.2](docs/design/digital-life-system-design.md).**
